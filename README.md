...
Variance-based filtering removes near-constant sensors below the threshold τ = 1e-6
Cumulative variance analysis retains features explaining at least 95–99% of the total variance
Coefficient of Variation (CV) ranking prioritises sensors with the highest relative fluctuation
...
the encoder outputs a compact latent representation 
𝑧
=
ℎ
𝑡
...
The predictive distribution is decomposed over 
𝐸
=
4
E=4 expert subnetworks:
...
i
 ...
 that captures operating-context information without requiring explicit domain labels.
...
2
	...
	Irreducible sensor and process noise
Total 
𝜎
𝑡
𝑜
...
2
	...
	Combined uncertainty used to compute 90% prediction intervals
...
2
2
 factorial design was used across all 13 training configurations to isolate the main drivers of cross-domain generalisation error. Ordinary Least Squares (OLS) regression on coded factors yields:
...
(all statistically significant at 
𝑝
<
0.001
p<0.001)
...
) is the dominant driver of cross-domain error in 8 of the 13 configurations, exerting a stronger effect than fleet size alone.
Fleet size exerts a protective effect (
𝛽
2
<
0
...
<0) when FD002 is included in the source pool, suggesting that larger multi-condition fleets help stabilise the gating vocabulary and reduce sensitivity to fault mismatch.
...
12
 ...
 is negative in 11 of 13 configurations, indicating that fault complexity and fleet size partially offset one another when both are present, thereby supporting the hypothesis that top-K gating encourages regime-coherent partitioning.
...
⚠️ Limitations
A residual monotonicity violation rate of 41.1% remains in the high-RUL plateau region (
RUL
>
100
RUL>100 cycles), where piecewise label discontinuities complicate the effect of a fixed-
𝜆
λ monotonicity penalty
...
Adaptive 
𝜆
λ scheduling to selectively strengthen monotonicity regularisation in the high-RUL plateau region
