...
> **Md. Salehin Seyama, Limon Bin Hossain**  
...
RUL targets are generated using a piecewise-linear degradation strategy with a cap of \( r_{\max} = 125 \) cycles, which reduces early healthy-phase noise and places greater emphasis on late-stage degradation behaviour.
...
1. Variance-based filtering removes near-constant sensors below the threshold \( \tau = 1 \times 10^{-6} \).
...
For each sliding window \( X_t \in \mathbb{R}^{w \times F} \):
...
\[
p(y \mid x) = \sum_i g_i(x)\, p_i(y \mid x)
\]
...
\[
S(w) = \lambda \sum \left| w_k(x) - w_k(x') \right|
\]
...
|-----------|---------------------------------------------------------|
| Epistemic \( \sigma_{\text{ep}}^2 \)   | Model uncertainty; reducible with more data         |
| Aleatoric \( \sigma_{\text{al}}^2 \)   | Irreducible sensor and process noise               |
| Total \( \sigma_{\text{tot}}^2 \)      | Combined uncertainty used for 90% prediction intervals |
...
| E       | + PIR + MC Dropout (Full)   | 19.38  | 15.25 | 0.870      |
...
|------------------------------------|--------------------------------|
| 104–125 cycles (early / plateau)   | E3 (gate weight ≈ 0.61)        |
| 62–83 cycles (transition region)   | E3 → E4 crossover              |
| 0–20 cycles (near end-of-life)     | E4 (gate weight ≈ 0.41)        |
...
A \( 2^2 \) factorial design was used across all 13 training configurations to isolate the main drivers of cross-domain generalisation error. Ordinary Least Squares (OLS) regression on coded factors yields:

\[
\text{RMSE} = \beta_0 + \beta_1 X_1 (\text{Fault Mode}) + \beta_2 X_2 (\text{Fleet Size}) + \beta_{12} X_1 X_2
\]
...
- Fault-mode complexity (\( \beta_1 \)) is the dominant driver of cross-domain error in 8 of the 13 configurations, exerting a stronger effect than fleet size alone.  
- Fleet size exerts a protective effect (\( \beta_2 < 0 \)) when FD002 is included in the source pool, suggesting that larger multi-condition fleets help stabilise the gating vocabulary and reduce sensitivity to fault mismatch.  
- The interaction term \( \beta_{12} \) is negative in 11 of 13 configurations, indicating that fault complexity and fleet size partially offset one another when both are present, thereby supporting the hypothesis that Top-K gating encourages regime-coherent partitioning.
...
- A residual monotonicity violation rate of 41.1% remains in the high-RUL plateau region (\(\text{RUL} > 100\) cycles), where piecewise label discontinuities complicate the effect of a fixed-\( \lambda \) monotonicity penalty.  
...
- Adaptive \( \lambda \) scheduling to selectively strengthen monotonicity regularisation in the high-RUL plateau region.  
